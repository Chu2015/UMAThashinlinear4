...
C     CALCULATE DDMDE
         IF ((DM .GT. DMOLD) .AND. (DM .LT. ONE)) THEN
		    ...
		    TERM = TERM1 + TERM2
	   		TERM3=TWO*GFMAT*(TERM-SQRT(TERM))
			    TERM4=TWO*GFMAT*TERM-CELENT*(STRESS(2)*STRANTM
     1			    + STRESS(3)*STRANT(3))
     ...
     2      		 +CDTHREE(2,2)*STRANT(2)+CDTHREE(3,2)*STRANT(3))
	  			ELSE IF(STRANT(2) .LE. ZERO) THEN
		 			TERM6=FOUR*GFMAT*(STRESS(2)*CDTHREE(2,2)/SIGTT**TWO +STRESS(3)
     1				 *CDTHREE(3,2)/SIGSLT**TWO)-CELENT*(CDTHREE(3,2)*STRANT(3))
  				END IF
     ...
     2      		+CDTHREE(3,1)*STRANT(3))
	  			ELSE IF(STRANT(2) .LE. ZERO) THEN
		 	 	TERM6=TWO*GFMAT*TWO*(STRESS(2)*CDTHREE(2,1)/SIGTT**TWO +STRESS(3)
     1				*CDTHREE(3,1)/SIGSLT**TWO)-CELENT*(CDTHREE(3,1)*STRANT(3))	
  				END IF
...
C     CALCULATE DDMDE
     ...
     1	     		 *STRANTM+STRESS(3)*STRANT(3))*(TWO*GAMA**TWO+
     ...
     7     +4.D0*BETA))*TERM2))
			  	ELSE IF(STRANT(2) .GE. ZERO) THEN
  		  TERM6 = 16.D0*GFMAT*BETA*TERM1-CELENT*((
     1     STRESS(3)+CDTHREE(3,3)*STRANT(3))
     2     *(TWO*GAMA**TWO+4.D0*BETA-TWO*GAMA*
     3     SQRT(GAMA**TWO+4.D0*BETA))+(ABS(STRESS(2))
     4     *ABS(STRANT(2))+STRESS(3)*STRANT(3))*((ONE-GAMA/
     5     SQRT(GAMA**TWO+4.D0*BETA))*4.D0*TERM1+(4.D0*GAMA-TWO*
...
C     CALCULATE DDFDE
            IF ((DF .GT. DFOLD) .AND. (DF .LT. ONE)) THEN
	  		  STRANTM = macauley(STRANT(1))
              TERM1 = TWO*GFFIB-CELENT*STRANTM
...
C     CALCULATE DF
          CALL DamageEvaluationfiber2( GFFIB, CELENT,
