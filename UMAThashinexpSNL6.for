...
C     shear nonlinearity
      ...
      IF (DMOLD .LE. ZERO) THEN
      	SHRLT = SHRLT2/(1+(SHRLT2*abs(STRANT(3))/SIGSLT)**6)**(1.0/6)       
      ELSE
        SHRLT = STATEV(14) 
      END IF 
c     
...
C     CALCULATE DFMNDE
         IF (DM .GT. DMOLD) THEN
	   		IF(STRANT(2) .GE. ZERO) THEN
				DFMNDE(2) = STRANT(2) / FMN / EPITT**TWO	
   	        ELSE IF(STRANT(2) .LT. ZERO) THEN
				DFMNDE(2) = STRANT(2) / FMN / EPICT**TWO	
            END IF
         
            IF (NDI .EQ. 3) THEN
               DFMNDE(4) = STRANT(4) / FMN / EPISLT**TWO
            ELSE IF (NDI .EQ. 2) THEN
               DFMNDE(4) = STRANT(3) / FMN / EPISLT**TWO
            END IF
            DO I = 1, 6
...
C     CALCULATE DFFNDE
           IF (DF .GT. DFOLD) THEN
		     IF  (STRANT(1) .GE. ZERO) THEN
			   DFFNDE(1) = STRANT(1) / FFN / EPITL**TWO
   	         ELSE IF(STRANT(1) .LT. ZERO) THEN
	           DFFNDE(1) = STRANT(1) / FFN / EPICL**TWO
   	         END IF
